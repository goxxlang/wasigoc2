...
| `websocket` (not `go list std` -- project extension) | **in** (`stdlib/websocket`: Go++ port of gorilla/websocket v1.5.3 — RFC 6455 Conn/Upgrader/Dialer/JSON, RFC accept-key GUID, client-to-server masking, ping→pong, close echo. `net/http` gained `Request.Header`/`Conn` and `Response.Hijack` so Upgrade can keep the TCP conn, matching gorilla's Hijacker without a ResponseWriter. Bounds: no DialContext/Proxy/TLS/wss, no permessage-deflate, no NextReader/NextWriter, no write deadlines, maskBytes is the appengine/safe loop. `websocketpkg` golden: Pipe handshake against the RFC 6455 sample key `dGhlIHNhbXBsZSBub25jZQ==` → accept `s3pPLMBiTxaQ9kYGzzhZRbK+xOo=`, text+JSON round trip, then wasigocvm Dial+Upgrade echo; stock wasip1 prints `http-skip`) |
| `liveview` (not `go list std` -- project extension) | **in** (`stdlib/liveview`: LiveView web framework, one language covering HTML, CSS, JS-events, and Jinja2/Go templates. `Parse`/`Component.Render`/`Invoke`/`Patch`, `Engine.Add`/`Mount`/`HandleEventJSON`, generated client JS that POSTs `/live/event` over HTTP/1.0 because this `net/http` has no WebSocket. `view` is HTML tags plus GuiKit GML (`div#id.class :gk-click."inc"`); templates are Go `{{ .Field }}`/`{{ if }}`/`{{ range }}` plus Jinja2 `{% if %}`/`{% for x in y %}`; events are the GS-shaped `Count += 1` / `dml.setValue` dialect. Interpolations HTML-escaped unless piped through safe. No wrapper/import filesystem, no contextual JS/CSS/URL autoescape. `liveviewpkg` golden: parse a counter (Go if + Jinja for + escape) and a GML box, invoke inc/dml, then on wasigocvm GET / + POST /live/event over real sockets; stock wasip1 prints `http-skip`) |
