...
| `net` | **in** (`Dial`/`Listen`/`Accept`/`Read`/`Write`/`Close` and packet counterparts are real via `gocvm.Call("net.*")` on `goclang++ --shim-sandbox` (Winsock) and on **wasigo-p2** (`compile.bat --wasip2`: in-module non-blocking sockets + `poll()`, see [wasip2.md](wasip2.md)). Plain wasm32-wasip1 still has no socket syscalls and falls back to the local Pipe stack. `Pipe()` works on every build.) |
| `net/http` | **in** (HTTP/1.0 `Get`/`Post`/`Serve`/`ServeMux` over `net` — Pipe on wasip1, real TCP on wasigo-p2 / native shim_sandbox) |
