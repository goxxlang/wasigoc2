#ifndef WASMNIX_INCLUDE_NIX_KVM_HOST_HPP_
#define WASMNIX_INCLUDE_NIX_KVM_HOST_HPP_
...
// vhost.h, and mshv.h — the Linux analog of WinHvPlatform /
...
inline bool kvm_close(int h) {
  KvmObj* p = kvm_at(h);
  if (!p) return false;
  if (p->kind == kKvmVm || p->kind == kKvmMshv) {
    KvmObj* t = kvm_tab();
    for (int i = 0; i < kKvmMax; ++i)
...
inline KvmObj* kvm_vm_of(int h) {
  KvmObj* p = kvm_at(h);
  if (!p) return nullptr;
  if (p->kind == kKvmVm || p->kind == kKvmMshv) return p;
...
inline bool kvm_cataloged(const char* api) {
  ...
  if (kvm_pfx(api, "KVM_") || kvm_pfx(api, "Kvm") || kvm_pfx(api, "VFIO_") ||
      kvm_pfx(api, "VHOST_") || kvm_pfx(api, "MSHV_")) {
    int n = 0;
...
inline bool try_kvm(const char* api, const char* args, std::string* out) {
  ...
  if (kvm_eq(api, "KVM_CREATE_VM") || kvm_eq(api, "MSHV_CREATE_PARTITION")) {
    int h = kvm_alloc(kvm_eq(api, "MSHV_CREATE_PARTITION") ? kKvmMshv : kKvmVm);
  ...
  if (kvm_eq(api, "KVM_CREATE_VCPU") || kvm_eq(api, "MSHV_CREATE_VP")) {
    ...
    KvmObj* p = kvm_at(vm);
    if (!p || (p->kind != kKvmVm && p->kind != kKvmMshv)) return fail(api);
  ...
  if (kvm_eq(api, "KVM_RUN") || kvm_eq(api, "MSHV_RUN_VP")) {
    std::string hs, idx;
    kvm_split(a, &hs, &idx);
    int h = static_cast<int>(std::strtol(hs.c_str(), nullptr, 10));
    KvmObj* vp = kvm_vcpu_of(h);
    if (!vp && kvm_at(h) && (kvm_at(h)->kind == kKvmVm || kvm_at(h)->kind == kKvmMshv)) {
      unsigned n = static_cast<unsigned>(std::strtoul(idx.c_str(), nullptr, 10));
      vp = kvm_vcpu(h, n);
      if (!vp) {
        int v = kvm_alloc(kKvmVcpu);
...
#endif  // WASMNIX_INCLUDE_NIX_KVM_HOST_HPP_
