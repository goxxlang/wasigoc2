...
// Note: real cppgc declares a RootVisitor-based TraceRootCallback here,
// used for conservative-stack-derived roots. This port is precise-only (see
// heap.h) and traces Persistent<T> roots through the same cppgc::Visitor
// interface as Member<T> edges, so TraceRootCallback (see
// cppgc/internal/persistent-node.h) is Visitor-based instead -- declaring
// both would conflict.
