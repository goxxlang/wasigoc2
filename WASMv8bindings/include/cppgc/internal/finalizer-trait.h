...
struct FinalizerTraitImpl<T, false> {
  static void Finalize(void* obj) {
    static_assert(sizeof(T), "T must be fully defined");
    static_assert(std::is_trivially_destructible_v<std::remove_cv_t<T>>);
    // Same Call(obj) shape as the non-trivial path: invoke the destructor
    // even when it is trivial so `obj` is the object being finalized.
    static_cast<T*>(obj)->~T();
  }
};
