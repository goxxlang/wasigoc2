...
class V8_EXPORT Platform {
 public:
   ...
   * task on the foreground thread. The default implementation returns a distinct
   * runner per priority that runs `PostTask` on the calling thread.
   */
  virtual std::shared_ptr<TaskRunner> GetForegroundTaskRunner(
   ...
   * \endcode
   *
   * This port's default (no thread pool on wasm32-wasip1) runs `job_task` on
   * the calling thread. `kUserBlocking` / `kUserVisible` run before
   * `PostJob` returns; `kBestEffort` waits until `Join()` (or handle
   * destruction).
   */
